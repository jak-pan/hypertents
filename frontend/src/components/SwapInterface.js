...
function SwapInterface({ sourceChain, sourceToken, destinationChain, destinationToken }) {
  ...
  const [recipient, setRecipient] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [swapInfoJson, setSwapInfoJson] = useState(''); // State to store the swap info JSON
  ...
  const handleSwap = () => {
    ...
    const swapInfoJsonString = JSON.stringify(swapInfo, null, 2);
    setSwapInfoJson(swapInfoJsonString);

    console.log('Swap Information:', swapInfoJsonString);

    // Save the JSON data or pass it where you need it here
    const existingSwaps = JSON.parse(localStorage.getItem('swaps')) || [];
    existingSwaps.push(swapInfo);
    localStorage.setItem('swaps', JSON.stringify(existingSwaps));
